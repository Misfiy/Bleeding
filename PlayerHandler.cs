namespace BleedingPlugin.Events
{
    ...
    {
        private readonly Config config = Bleeding.Instance.Config;
        public void OnHurting(HurtingEventArgs ev)
        {
            string attacker = ev.Attacker != null ? ev.Attacker.Role.Type.ToString() : "No attacker";
            Log.Debug($"{ev.Player.Nickname} was hurt: {ev.DamageHandler.Type}. Attacker: {attacker}");
            if (ev.Player.IsScp && !config.CanScpsBleed) return;
